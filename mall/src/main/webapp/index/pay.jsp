...
<jsp:include page="header-login.jsp"/>
	<div id="main">
		...
		<div id="goodslist">
    		...
    		<div class="list-cont">
				<c:forEach items="${orders.items}" var="items">
					<ul>
	                    <li class="li1">
	                        <dl>
	                            <a href="detail?id=${items.goods.id}">
	                            	<dt><img src="${items.goods.cover}"></dt>
	                                <dd class="goodsname" style="padding-top: 36px;">${items.goods.name}</dd>
	                                <dd class="goods-des">${items.goods.intro}</dd>
	                            </a>
	                        </dl>
	                    </li>
	                    <li class="li3">￥<span>${items.goods.price}</span></li>
	                    <li class="li4">x<span>${items.amount}</span></li>
	                    <li class="li5">￥<span>${items.goods.price*items.amount}</span></li>
	                </ul>
                </c:forEach>
			</div>
	        <p class="zprice" style="text-align: right;">应付总金额：
	        	<span>￥<span id="shifu" style="font-size: 32px;font-weight: bold;color: #FF712B;">${orders.total}</span></span>
	        </p>
		</div>
		    ...
		    <div class="collect-cont new-address" style="padding: 0px 0px;">
				<form action="orderPay" method="post" id="form">
			        ...
			        <div class="box clearfix">
						...
						<div class="right">
							<input type="text" id="name" name="name" value="${userInfo.name}" placeholder="请输入收货人姓名">
					...
					<div class="box clearfix">
						...
						<div class="right">
							<input type="text" id="phone" name="phone" value="${userInfo.phone}" placeholder="请输入收货人电话">
						<span class="Validform_checktip"></span></div>
					</div>
					<div class="box clearfix">
						<div class="left">
						...
						<div class="right">
							<input type="text" id="address" name="address" value="${userInfo.address}" placeholder="请输入收货地址">
						</div>
					</div>
				</form>
		    </div>
